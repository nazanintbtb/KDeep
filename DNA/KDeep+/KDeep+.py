...
# Convolutional Layer
  ...
  checkpointer = keras.callbacks.ModelCheckpoint(filepath="./model/KDeepp.h5",monitor='val_loss', verbose=1, save_best_only=True)
  earlystopper = keras.callbacks.EarlyStopping(monitor='val_loss', patience=6, verbose=1)




  model.fit(get_train_data(100), steps_per_epoch=int(np.ceil(4400000/100)), epochs=60,
                        validation_data=get_valid_data(100), validation_steps=int(np.ceil(8000/100)), callbacks=[checkpointer,earlystopper])

  model.save('./model/KDeepp.h5')
#-----------------------------------------------------------------------
